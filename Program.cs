...
namespace Squid_Game
{
    ...
    public class Dice
    {
        ...
        // Δημιουργία μεθόδου, η οποία θα μού επιστρέφει κατά το ρίξιμο του μαγικού ζαριού, τον επόμενο τυχαίο ακέραιο αριθμό που θα παραχθεί, καθώς καλείται στην κλάση SquidGame.
        public int DiceRoll()
        {
    ...
    // Κλάση υπεύθυνη για την διάδραση με τον χρήστη και τον έλεγχο των προσπαθειών.
    public class SquidGame
    {
        ...
        public void StartGame()   
        {
            ...
            // στην μεταβλητή GameTarget, που είναι αυτή που θα περιέχει τον τυχερό αριθμό.
            ...
            Χρησιμοποιώ αυτή την μεταβλητή numberOfTriesLeft, ως μετρητή για να μπορώ να μετράω τις υπόλοιπες προσπάθειες που έχει ο χρήστης και να τις μειώνω
            κατά 1, κάθε φορά που δεν βρίσκει τον τυχερό αριθμό καθώς και να ελέγχεται εάν του έχουν περισέψει προσπάθειες για να συνεχιστεί το παιχνίδι και να του
            ζητείται ξανά η εκχώρηση επόμενου ακέραιου αριθμού και να τον ενημερώνει για το πόσες συνολικά προσπάθειες του έχουν απομείνει.
            */
            ...
            while(numberOfTriesLeft > 0)    // Για όσο οι προσπάθειες που έχουν απομείνει στον χρήστη είναι > 0.
            {
                ...
                if (!int.TryParse(userInput, out int numberOfGuess))    // Προσπάθεια μετατροπής του αριθμού σε ακέραιο και εξαγωγή αυτού στην μεταβλητή numberOfGuess.
                {
                    ...
                    continue;   // Οι παρακάτω εντολές παρακάμπτονται και συνεχίζεται να εκτελείται ο τρέχον βρόγχος με το να ζητάει από τον χρήστη να εισάγει έναν αριθμό ξανά.
                }
                if(numberOfGuess < 1 || numberOfGuess > 12)   // Σε δεύτερο έλεγχο τσεκάρω εάν ο αριθμός που εισήγαγε ο χρήστης είναι μεταξύ 1 εώς 12, ειδάλλως εάν το ένα από τα 2 σκέλη της συνθήκης μού δώσει αποτέλεσμα true, τότε μπαίνω μέσα στην if και ζητάω από τον χρήστη να μού εισάγει ξανά αριθμό και συνεχίζεται η ροή του προγράμματος χωρίς να μειώνονται οι προσπάθειες. 
                {
                ...
                if (numberOfGuess == GameTarget)    // Εάν ο αριθμός που έχει δώσει ο χρήστης κατά την είσοδο ισοδυναμεί με τον τυχερό αριθμό (είναι πράγματι ο τυχερός αριθμός) που βρίσκεται μέσα στην μεταβλητή GameTarget, τότε ο χρήστης ενημερώνεται ότι κέρδισε το παιχνίδι. 
                {
                    Console.WriteLine("Well done! You Won the game!");
                    return;   // Εδώ τερματίζεται η εκτέλεση του προγράμματος (με την εντολή return), καθώς δεν θα εκτελεστούν οι παρακάτω εντολές, γιατί ο χρήστης κέρδισε το παιχνίδι.
                }
                else
                {
                    numberOfTriesLeft--;    // Μείωση των προσπαθειών που έχουν απομείνει στον χρήστη κατά 1, στην περίπτωση που το numberOfGuess (ο αριθμός που εισήγαγε ο χρήστης) δεν ισοδυναμεί/δεν είναι ο τυχερός αριθμός που βρίσκεται μέσα στην μεταβλητή GameTarget κι έτσι παίρνω false από την συνθήκη της if και δεν εκτελείται το σώμα αυτής και εκτελείται το else.
                if (numberOfTriesLeft > 0)   // Εδώ ελέγχω εάν ο αριθμός των εναπομείναντων προσπαθειών του χρήστη είναι > 0 τότε παίρνω true από την συνθήκη και τον ενημερώνω ότι ήταν λάθος η μαντεψιά του και τον ενημερώνω για το πόσες προσπάθειες του έχουν απομείνει καθώς εκτελείται ξανά το σώμα της while στην περίπτωση που ο αριθμός των προσπαθειών του χρήστη είναι γνησίως > 0. Εάν από την συνθήκη πάρω false (που σημαίνει ότι ο χρήστης δεν έχει άλλες προσπάθειες και οι εναπομείναντες προσπάθειες είναι 0, τότε δεν υλοποιείται το σώμα της if και βγαίνω και από τον επαναληπτικό βρόγχο και δεν ζητάω αριθμό από τον χρήστη, καθώς δεν έχει άλλες προσπάθειες.
                {
                    Console.WriteLine($"Wrong Guess! Try again! You have {numberOfTriesLeft} tries left!");
                }
                }
            }
            // Ενημέρωση του χρήστη ότι έχασε το παιχνίδι (αφού δεν έχει και άλλες προσπάθειες για να συνεχιστεί το παιχνίδι => παίρνω false από την συνθήκη της while και δεν ζητάω από τον χρήστη να εισάγει αριθμό) και ενημέρωσή του για το ποιος ήταν ο τυχερός κρυφός αριθμός, που φυλάσσεται μέσα στην μεταβλητή GameTarget.
